...
# All rows with run > 30
significant = np.where(longest_run > 30)[0]
...
for r in significant[1:]:
    if r - groups[-1][-1] <= 3:
        ...
        groups.append([r])
print(f"Groups (run > 30): {len(groups)}")
for g in groups:
    print(f"  rows {g[0]}-{g[-1]}  (width {g[-1]-g[0]+1})  max_run={int(longest_run[g[0]:g[-1]+1].max())}")
